...
class ShapeNetPartDataset(Dataset):
    ...
    def __init__(self, class_name, input_threads=8, train_i=[1,2,3,4,5], test_i=[6,7], train_samples=1):
        ...
        if self.category == 'MAIZE':
          self.path = 'Data/pheno4d_kpconv_maize'
        elif self.category == 'TOMATO':
          self.path = 'Data/pheno4d_kpconv_tomato'
        else:
          self.path = 'Data/pheno4d_kpconv'
    ...
    def prepare_ShapeNetPart_ply(self):
        ...
        if self.category == 'MAIZE':
          category_and_synsetoffset = [['Maize', 'Maize']]
        elif self.category == 'TOMATO':
          category_and_synsetoffset = [['Tomato', 'Tomato']]
        else:
          category_and_synsetoffset = [['Maize', 'Maize'],
                                      ['Tomato', 'Tomato']]
        ...
        # Collect splits
        # **************
        train_i, test_i = self.train_i, self.test_i
        if self.category == 'MAIZE':
          train_models = ['M0'+str(i) for i in train_i]
          test_models = ['M0'+str(i) for i in test_i]
        elif self.category == 'TOMATO':
          train_models = ['T0'+str(i) for i in train_i]
          test_models = ['T0'+str(i) for i in test_i]
        else:
          train_models = ['M0'+str(i) for i in train_i] + ['T0'+str(i) for i in train_i]
          test_models = ['M0'+str(i) for i in test_i] + ['T0'+str(i) for i in test_i]
        print('>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>')
        print(train_models)
        print(test_models)
        print('>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>')
        ...
        train_files = []
        test_files = []
        for item in category_and_synsetoffset:
            dir_point = os.path.join(self.path, item[1], 'points')
            print(dir_point)
            fns = sorted(os.listdir(dir_point))
            #print(fns[0][0:3])
            prefix = item[1] + '/'
            train_files += [prefix+fn[0:-4] for fn in fns if ((fn[0:3] in train_models))]
            test_files += [prefix+fn[0:-4] for fn in fns if ((fn[0:3] in test_models))]
        

        train_files, val_files = train_test_split(train_files, test_size=0.01, random_state=42)
        nsample = math.ceil(len(train_files) * self.train_samples)
