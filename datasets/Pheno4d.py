...
class Pheno4dDataset(Dataset):
    """
    Pheno4d dataset for segmentation task. Can handle both unique object class or multi classes models.
    """
    ...
    def __init__(self, class_name, input_threads=8):
        """
        Initiation method. Give the name of the object class to segment (for example 'Maize') or 'multi' to segment
        all objects with a single model.
        """
        Dataset.__init__(self, 'Pheno4d_' + class_name)
        ...
        self.prepare_Pheno4d_ply()

        return

    def split_data_by_plant(self, plant:str, random_state):
        letter = 'M' if plant.upper() == 'MAIZE' else 'T'
        file_list = self.getListOfFilesByPlant(self.path,plant)
        plants = [1,2,3,4,5,6,7]
        train_idx, test_idx = train_test_split(plants, test_size=0.2, random_state = random_state)
        tmp,val_idx = train_test_split(train_idx,test_size=0.2,random_state=random_state)
        train = []
        val =[]
        test = []
    ...
    def getListOfFilesByPlant(self,dirName, plant:str):
        letter = 'M' if plant.upper() == 'MAIZE' else 'T'
    ...
    def prepare_Pheno4d_ply(self):
        ...
        split_files = [train_maize+train_tomato, val_maize+val_tomato, test_maize+test_tomato]
        ...
        for split, files in zip(splits, split_files):
            ...
            N = len(files)
            class_nums = {n: 0 for n in categories}
