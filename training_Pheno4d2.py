...
if __name__ == '__main__':
    ...
    train_i, test_i = train_test_split(models, test_size=2, random_state=42)
    dataset = ShapeNetPartDataset(config.dataset.split('_')[1], config.input_threads, train_i, test_i, .01)

    # Create subsampled input clouds
    dl0 = config.first_subsampling_dl
    dataset.load_subsampled_clouds(0)
